...
`callable_`: Workflows that can be invoked by other repositories (generally as part of their dependency-evaluation process).

`event_`: Workflows that are triggered by an event (pr, push, periodic, etc.).

`impl_`: Callable workflows that provided functionality used in the implementation of a repository's main validation process.
